  ...
  return client.query(
    `INSERT INTO ${tableName}
        (app_nino, app_title, app_first_name, app_last_name, app_dob, app_address1,
         app_address2, app_address3, app_town, app_county, app_postcode
        )
        VALUES
        ('${claimantObject.app_nino}', '${claimantObject.title}', '${claimantObject.first_name}',
         ...
         '${claimantObject.address2}', '${claimantObject.address3}', '${claimantObject.town}',
         '${claimantObject.county}', '${claimantObject.postcode}')`
  );
};

const insertBankDetailsintoDB = (claimantObject) => {
  let tableName = "payment_bank_details";
  return client.query(
    `INSERT INTO ${tableName}
        (bank_name, account_no, sort_code
        )
        VALUES
        ('${claimantObject.bank_name}', '${claimantObject.account_no}',
         '${claimantObject.sort_code}')`
  );
};

const insertPensionDetailsintoDB = (claimantObject) => {
    let tableName = 'claimant_pension_details'
    return client.query(
        `INSERT INTO ${tableName}
        ...
        VALUES
        (1, ${claimantObject.pension_amount},${claimantObject.frequency},
