    ...
    .then(result => {
        if(result.rowCount == 0) {
            ...
            if(result.rows[0].nino === newString) {
                session.claimantNino = newString.toUpperCase()
...
const setAppointeeInClassToBlank = (dbDetails) => {
    claimantsAppointee.setNino = '';
    claimantsAppointee.setTitle = '';
    claimantsAppointee.setFirstName = '';
    claimantsAppointee.setLastName = '';
    claimantsAppointee.setDOB = '';
    claimantsAppointee.setAddress1 = '';
    claimantsAppointee.setAddress2 = '';
    claimantsAppointee.setAddress3 = '';
    claimantsAppointee.setTown = '';
    claimantsAppointee.setCounty = '';
    claimantsAppointee.setPostcode = '';
}
...
const processAddAppointee = (request, response) => {
    let session = request.session;
    let formApp_nino = request.body.app_nino.toUpperCase()
    request.body.app_nino = request.body.app_nino.toUpperCase();
    webDbController.insertAppointeeDetailsintoDB(request.body)
        .then(result => {
            webDbController.getAppointeeID(formApp_nino)
            .then(result => {
               let appointeeID = result.rows[0].id
                let session_claimant_id = session.claimantID
                webDbController.updateClaimantWithAppointeeDetails(session_claimant_id, appointeeID)
                .then(result => {
                    response.render('addBankDetails');
                })
                .catch(error => {
                    console.log('Error Inserting Appointee Details into Claimant record' + error)
                })
            })
            .catch(error => {
                console.log('Error getting Appointee ID ' + error)
            })
        })
        .catch(error => {
...
const processAddBankDetails = (request, response) => {
    ...
    let session = request.session
    console.log(session)
    let sessionCustomerID = session.claimantID
    webDbController.insertBankDetailsintoDB(request.body, sessionCustomerID)
    .then(result => {
        console.log('done')
        ...
        .then(result => {
            ...
            .then(result => {
                ...
                response.render('addPensionDetails', {'penTypes': penTypes, 'penFreqs': penFreqs})
            })
            .catch(error => {
                console.log('Error retriveing Pension Frequency for Add Bank Details' + error)
            })
        })
        .catch(error => {
            console.log('Error getting Pension Types for Add Bank Details' + error)
        })
    })
    .catch(error => {
        console.log('Error inserting App Deets' + error)
...
const processAddPensionDetails = (request, response) => {
    let session = request.session
    let sessionCustomerID = session.claimantID
    webDbController.insertPensionDetailsintoDB(request.body, sessionCustomerID)
        .then(result => {
...
const processAddEditClaimant = (request, response) => {
    ...
    if(session.claimantNino == null) {
        response.render('editClaimantDetails', {items: []})
    } else {
        ...
        .then(result => {
            ...
            .then(result => {
                response.render('editClaimantDetails', {items: clDetails.rows[0], secQuestions:result.rows})
            })
        })
    }
}
...
const processAddEditAppointee = (request, response) => {
    let session = request.session
    if(session.claimantNino == null) {
        response.render('editAppointeeDetails', {items: []})
    } else {
        webDbController.getAppointeeUserDetails(session.claimantNino)
        .then(result => {
            response.render('editAppointeeDetails', {items: result.rows[0]})
        })
    }
}
...
const processUpdateClaimant2 = (request, response) => {
   let session = request.session
   request.body.nino = request.body.nino.toUpperCase();
   if(request.body.appointee === 'Yes' || request.body.appointee === 'Y') {
    console.log(request.body)
        webDbController.insertClaimantDetailsintoDB(request.body)
        .then(result => {
            webDbController.getClaimantIDForSession(request.body.nino)
            .then(result => {
                console.log('Claimant ID is : ' + result.rows[0].id)
                
                session.claimantID = result.rows[0].id
                response.render('addAppointeeDetails');
            })
            .catch(error => {
                console.log('Error getting Claimant ID for Session' + error)
            })
        })
        .catch(error => {
