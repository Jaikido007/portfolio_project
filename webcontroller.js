...
const processSearchClaimant = (request, response) => {
  ...
  webDbController
    ...
    .then((result) => {
      if (result.rowCount == 0) {
        ...
        if (result.rows[0].nino === newString) {
          ...
          webDbController
            .getSecurityQuestions(result.rows[0].nino)
            .then((result) => {
            ...
            .catch((error) => {
              console.log(
                `${chalk.red(
                  "Error: processSearchClaimant 1st catch " + error
                )}`
              );
            });
        } else {
    ...
    .catch((error) => {
      console.log(
        `${chalk.red("Error: processSearchClaimant 2nd catch " + error)}`
      );
    });
};
...
const processSecurityClearence = (request, response) => {
  let dobNum = verifyDOB(
    request.body["dob-day"],
    request.body["dob-month"],
    request.body["dob-year"]
  );
  session = request.session;
  console.log("Process security clearance: clid = " + session.claimantID);
  let nino = session.claimantNino;
  webDbController.verifySecurityDetails(nino).then((result) => {
    if (
      dobNum != result.rows[0].dob ||
      request.body.sec1 != result.rows[0].sec_answer1 ||
      request.body.sec2 != result.rows[0].sec_answer2
    ) {
...
const populateClaimantInClass = (dbDetails) => {
  ...
  searchedBenefitClaimant.setSecurityQuestion1 =
    dbDetails.rows[0].sec_question1;
  searchedBenefitClaimant.setSecurityQuestion2 =
    dbDetails.rows[0].sec_question2;
...
const processUpdateClaimant = (request, response) => {
  ...
  webDbController
    .updateClaimantInDB(nino, request.body)
    .then((result) => {
      processClaimantDetails(request, response);
    })
    .catch((error) => {
...
const processAddAppointee = (request, response) => {
  //fix NI to upper
  //add to claimant record
  webDbController
    .insertAppointeeDetailsintoDB(request.body)
    .then((result) => {
      response.render("addBankDetails");
    })
    .catch((error) => {
      console.log("Error inserting App Deets" + error);
    });
  console.log("done");
};
...
const processUpdateAppointee = (request, response) => {
  ...
  if (bool_flag === true) {
    ...
    webDbController
      .updateAppointeeInDB(nino, request.body)
      .then((result) => {
...
const processAddBankDetails = (request, response) => {
  console.log(request.body);
  webDbController
    .insertBankDetailsintoDB(request.body)
    .then((result) => {
      console.log("done");
      webDbController
        .getPensionTypes()
        .then((result) => {
          let penTypes = result.rows;
          console.log(penTypes);
          webDbController
            .getPensionFrequency()
            .then((result) => {
              let penFreqs = result.rows;
              response.render("addPensionDetails", {
                penTypes: penTypes,
                penFreqs: penFreqs,
              });
            })
            .catch((error) => {
              console.log(
                "Error retriveing Pension Frequency for Add Bank Details" +
                  error
              );
            });
        })
        .catch((error) => {
          console.log(
            "Error getting Pension Types for Add Bank Details" + error
          );
        });
    })
    .catch((error) => {
...
const processUpdateBankDetails = (request, response) => {
  ...
  webDbController
    .updateBankDetailsInDB(clid, request.body)
    .then((result) => {
...
const processAddPensionDetails = (request, response) => {
  webDbController
    .insertPensionDetailsintoDB(request.body)
    .then((result) => {
...
const processUpdatePensionDetails = (request, response) => {
  ...
  let clid = session.claimantID;
  webDbController
    .updatePensionDetailsInDB(clid, request.body)
    .then((result) => {
      processPensionDetails(request, response);
    })
    .catch((error) => {
      console.log(
        `${chalk.red("Error: processUpdatePensionDetails " + error)}`
      );
    });
};
...
const processAddEditClaimant = (request, response) => {
  ...
  if (session.claimantNino == null) {
    ...
    webDbController
      .getClaimantUserDetails(session.claimantNino)
      .then((result) => {
        let clDetails = result;
        webDbController.getAllSecurityQuestions().then((result) => {
          response.render("addClaimantDetails", {
            items: clDetails.rows[0],
            secQuestions: result.rows,
          });
        });
      });
  }
...
const processAddEditAppointee = (request, response) => {
  ...
  if (session.claimantNino == null) {
    ...
    webDbController
      .getAppointeeUserDetails(session.claimantNino)
      .then((result) => {
...
const processAddEditBankDetails = (request, response) => {
  ...
  if (session.claimantNino == null) {
    ...
    webDbController.getBankUserDetails(session.claimantNino).then((result) => {
...
const processAddEditPensionDetails = (request, response) => {
  ...
  if (session.claimantNino == null) {
    ...
    webDbController
      .getPensionUserDetails(session.claimantNino)
      .then((result) => {
        ...
        webDbController
          ...
          .then((result) => {
            ...
            webDbController
              ...
              .then((result) => {
                response.render("addPensionDetails", {
                  items: penDetails.rows[0],
                  penTypes: penTypes.rows,
                  penFreqs: result.rows,
                });
              })
              .catch((error) => {
                console.log(
                  `${chalk.red("Error: getPensionFrequency " + error)}`
                );
              });
          })
          .catch((error) => {
...
const processWeekly = (request, response) => {
  webDbController
    .getPaymentFrequencyID("Weekly")
    .then((result) => {
      let payFrequency = result.rows[0].id;
      webDbController.getClaimantsReqPayment(payFrequency).then((result) => {
        if (result.rowCount === 0) {
          response.render("paymentRunOptions", {
          ...
          for (let j = 0; j <= i - 1; j++) {
            ...
            webDbController
              ...
              .catch((error) => {
                console.log(
                  `${chalk.red(
                    "Error: Couldn't insert payment details " + error
                  )}`
                );
              });
          }
          response.render("paymentRunOptions", {
            message: "Payment run succesful",
          });
        }
      });
    })
    .catch((error) => {
      console.log(
        `${chalk.red("Error: getting pension frequency ID" + error)}`
      );
    });
};
...
const processNewClaimant = (request, response) => {
  webDbController.getAllSecurityQuestions().then((result) => {
    response.render("addClaimantDetails", { secQuestions: result.rows });
  });
};

const processUpdateClaimant2 = (request, response) => {
  console.log(request.body);
  if (request.body.appointee === "Yes" || request.body.appointee === "Y") {
    webDbController
      .insertClaimantDetailsintoDB(request.body)
      .then((result) => {
        response.render("addAppointeeDetails");
      })
      .catch((error) => {
        console.log("Error inserting Cl Deets" + error);
      });
  } else {
    webDbController
      .insertClaimantDetailsintoDB(request.body)
